...
void go2_controller::Create_Jacobian(const Eigen::VectorXd& q)
{
    ...
    Eigen::Matrix4d A_ROTATE = Transformation_Matrix(M_PI/2.0, 0, 0, M_PI/2.0);
    ...
    // FL 구하기
    Eigen::Matrix4d A_thigh_FL = A_hip_FL * Transformation_Matrix(q(0)+M_PI/2.0, 0, -hip_to_thigh, M_PI/2.0);
    Eigen::Matrix4d A_calf_FL = A_thigh_FL * Transformation_Matrix(q(1), 0, -thigh_to_calf, 0);
    Eigen::Matrix4d A_ee_FL = A_calf_FL * Transformation_Matrix(q(2), 0, -calf_to_foot, 0);

    // FR 구하기
    Eigen::Matrix4d A_thigh_FR = A_hip_FR * Transformation_Matrix(q(3)+M_PI/2.0, 0, hip_to_thigh, M_PI/2.0);
    Eigen::Matrix4d A_calf_FR = A_thigh_FR * Transformation_Matrix(q(4), 0, thigh_to_calf, 0);
    Eigen::Matrix4d A_ee_FR = A_calf_FR * Transformation_Matrix(q(5), 0, calf_to_foot, 0);

    // RL 구하기
    Eigen::Matrix4d A_thigh_RL = A_hip_RL * Transformation_Matrix(q(6)+M_PI/2.0, 0, -hip_to_thigh, M_PI/2.0);
    Eigen::Matrix4d A_calf_RL = A_thigh_RL * Transformation_Matrix(q(7), 0, -thigh_to_calf, 0);
    Eigen::Matrix4d A_ee_RL = A_calf_RL * Transformation_Matrix(q(8), 0, -calf_to_foot, 0);

    // RR 구하기
    Eigen::Matrix4d A_thigh_RR = A_hip_RR * Transformation_Matrix(q(9)+M_PI/2.0, 0, hip_to_thigh, M_PI/2.0);
    Eigen::Matrix4d A_calf_RR = A_thigh_RR * Transformation_Matrix(q(10), 0, thigh_to_calf, 0);
    Eigen::Matrix4d A_ee_RR = A_calf_RR * Transformation_Matrix(q(11), 0, calf_to_foot, 0);
