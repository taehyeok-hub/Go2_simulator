...
void go2_controller::TaskSpacePDControl(double Kp, double Kd)
{
    ...
    Eigen::Matrix<double, 6, 12> jacobian = KINE.Get_Jacobian();
    ...
    Eigen::Vector3d Force_FL = Kp_Task * (EE_Pose_FL_desired - ee_pose[FL]) + Kd_Task * (EE_Vel_FL_desired - EE_Vel_FL);
    Eigen::Vector3d Force_FR = Kp_Task * (EE_Pose_FR_desired - ee_pose[FR]) + Kd_Task * (EE_Vel_FR_desired - EE_Vel_FR);
    Eigen::Vector3d Force_RL = Kp_Task * (EE_Pose_RL_desired - ee_pose[RL]) + Kd_Task * (EE_Vel_RL_desired - EE_Vel_RL);
    Eigen::Vector3d Force_RR = Kp_Task * (EE_Pose_RR_desired - ee_pose[RR]) + Kd_Task * (EE_Vel_RR_desired - EE_Vel_RR);
