...
void go2_controller::Forward_Kinematics(const Eigen::VectorXd& q,const Eigen::VectorXd& dq)
{
    // 변수
...
const double body_length = 0.1934; // 0.19275
const double body_width = 0.0465; // 0.145
const double l1 = 0.0955; // 0.085
const double l2 = 0.213; // 0.215
const double l3 = 0.213; // 0.215

// 함수 원형 선언
Eigen::Matrix4d A_Matrix_(double a, double alpha, double d, double theta);
// A_Matrix라는 함수가 무엇인지 모르는 상태에서 Forward_Kinematics 함수 안에서 사용하려고 했기 때문에 발생합니다.

void go2_controller::Forward_Kinematics_ME(const Eigen::VectorXd& q, const Eigen::VectorXd& dq)
{
    ...
    Eigen::Matrix4d AM = A_Matrix_(0, pi/2.0, 0, pi/2.0);
    ...
    // FL
    A2 = A_Matrix_(-0.085, pi/2.0, 0, pi/2.0+q(0));
    A3 = A_Matrix_(-0.215, 0, 0, q(1));
    A4 = A_Matrix_(-0.215, 0, 0, q(2));

    T1 = A1_FL * AM * A2 * A3 * A4;

    // FR
    A2 = A_Matrix_(0.085, -pi/2.0, 0, -pi/2.0+q(3));
    A3 = A_Matrix_(0.215, 0, 0, q(4));
    A4 = A_Matrix_(0.215, 0, 0, q(5));

    T2 = A1_FR * AM * A2 * A3 * A4;

    // RL
    A2 = A_Matrix_(-0.085, pi/2.0, 0, pi/2.0+q(6));
    A3 = A_Matrix_(-0.215, 0, 0, q(7));
    A4 = A_Matrix_(-0.215, 0, 0, q(8));

    T3 = A1_RL * AM * A2 * A3 * A4;

    // RR
    A2 = A_Matrix_(0.085, -pi/2.0, 0, -pi/2.0+q(9));
    A3 = A_Matrix_(0.215, 0, 0, q(10));
    A4 = A_Matrix_(0.215, 0, 0, q(11));
